...
public class Listar_Reservas extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<DAOReserva> listarTodos = cr.ListarTodos();
        ...
        if (!listarTodos.isEmpty()) {
            ...
            for (int i = 0; i < listarTodos.size(); i++) {
                respuesta += elementBuilder(listarTodos.get(i));
            }
            respuesta += "</table>";
            request.setAttribute("respuesta", "<div class='formulario'>" + respuesta + "</div>");
            request.getRequestDispatcher("Listar_Reservas.jsp").forward(request, response);
        } else {
